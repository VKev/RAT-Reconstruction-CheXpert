...
def main():
    ...
    parser.add_argument("--chexpert-only-support-devices", action="store_true",
                        help="Use only samples with Support Devices label (overrides exclude if both set)")
    ...
    dm = ImageDataModule(
        ...
        chexpert_only_support_devices=args.chexpert_only_support_devices,
    ...
    if args.dataset == "chexpert":
        ...
        try:
            x, y = next(islice(iter(loader), allowed - 1, allowed))
    ...
    print(f"[visualize] Model: {'RAT' if args.dataset in ('flowers102','chexpert') else 'SmallVAE'}")
