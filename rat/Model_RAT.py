...
class RAT(nn.Module):
    ...
    def forward(self, inp, mask_original, label=None): 
        ...
        mask = F.interpolate(mask_original.float().unsqueeze(1),  # ← make it float
                     scale_factor=1/self.mask_scale,
                     mode='nearest') 
...
if __name__ == "__main__":
    model = RAT(
        scale=1,                 # keep spatial size unchanged
        img_channel=3,           # 3-channel input
        width=64,                # embedding width
        middle_blk_num=12,
        enc_blk_nums=[2, 2],
        dec_blk_nums=[2, 2],
        loss_fun=None            # inference-only demo
    ...
    mask = torch.randint(low=0, high=5, size=(batch_size, H, W)).float()  # <- cast here
