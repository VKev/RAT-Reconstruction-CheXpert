...
def get_recon(x):
    """Extract the reconstruction tensor from diverse model outputs.

    Accepts tensor, list/tuple, or dict outputs and returns the primary reconstruction tensor.
    """
    ...
    if isinstance(x, dict):
        for k in ("recon", "x_hat", "output", "reconstruction"):
...
def tensor2img(tensor, min_max=(0, 1), out_type=np.uint8): 
    ...
    img_np = tensor.clamp_(*min_max).numpy() 
